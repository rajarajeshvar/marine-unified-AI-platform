...
export default function Home() {
  ...
  const fetchMaintenance = useCallback(async () => {
    try {
      const res = await fetch('http://localhost:8000/maintenance?limit=5');
      if (res.ok) {
        const data = await res.json();
  ...
  // FIX #7: Determine if records look like maintenance or events
  const getEventLabel = () => {
  ...
  return (
    <div className="app-shell">
      ...
      <aside className="left-sidebar">
        ...
        {/* FIX #7: Recent Events / Recent Maintenance — contextual label */}
        <div className="sidebar-section">
          ...
          <div className="chat-messages">
            {messages.map(msg => (
              <div key={msg.id} className={`msg-row ${msg.role}`}>
                ...
                {/* Message Actions — FIX #4: Added Show Evidence */}
      ...
      <aside className="right-sidebar">
        {telemetryConnected && sensorData ? (
          <>
            ...
            {/* AI Prediction — FIX #6: Only in right panel, removed from left */}
            {predictions && (
              <div className="right-section">
                <div className="right-section-header">
