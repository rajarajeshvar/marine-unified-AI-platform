...
import Map from "../../components/route-optimization/Map";
import Analytics from "../../components/route-optimization/Analytics";
import Recommendations from "../../components/route-optimization/Recommendations";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (urlParams.get('emergency') === 'true') {
        triggerEmergency(selectedShip);
      }
    }
  }, []);
  
  const triggerEmergency = (ship: any) => {
    setIsEmergency(true);
    const currentLat = ship?.lat || 25.0;
    const currentLng = ship?.lng || -40.0;
  ...
  const fetchRoute = async (startLat = 25.0, startLng = -40.0, destLat = 40.0, destLng = -20.0) => {
    ...
    try {
      ...
      const recsResponse = await fetch(`http://localhost:8002/recommendations?lat=${startLat}&lng=${startLng}`);
  ...
  // Handle Emergency Trigger from Engine Health
  useEffect(() => {
    if (engineHealth && engineHealth.failure_probability > 90 && !isEmergency) {
      triggerEmergency(selectedShip);
    }
  }, [engineHealth, isEmergency, selectedShip]);

  useEffect(() => {
    let isEmergencyUrl = false;
    if (typeof window !== 'undefined') {
      const urlParams = new URLSearchParams(window.location.search);
      isEmergencyUrl = urlParams.get('emergency') === 'true';
    }
  ...
  return (
    <div className={`min-h-screen p-6 transition-colors duration-1000 ${isEmergency ? 'bg-red-950 text-red-50' : 'bg-slate-900 text-slate-200'}`}>
      ...
      <header className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-4">
          <button 
            onClick={() => {
              if (typeof window !== 'undefined') {
                window.location.href = '/?acknowledged=true';
              }
            }}
      ...
      {/* START AND STOP COORDINATES DISPLAY */}
      {routeData && routeData.best_route && routeData.best_route.length > 0 && (
        <div className={`p-4 rounded-lg border mb-6 flex flex-col md:flex-row items-center justify-between gap-4 ${isEmergency ? 'bg-red-900/20 border-red-800' : 'bg-slate-800 border-slate-700'}`}>
          <div className="flex flex-col items-center md:items-start">
            <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Origin</span>
            <span className="font-mono text-lg text-blue-300">
              {routeData.best_route[0].lat.toFixed(4)}°, {routeData.best_route[0].lng.toFixed(4)}°
            </span>
          </div>
          
          <div className="flex-1 w-full flex items-center px-4">
            <div className={`h-px w-full relative ${isEmergency ? 'bg-red-700' : 'bg-slate-600'}`}>
              <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-3 text-xs tracking-widest ${isEmergency ? 'bg-red-950 text-red-400' : 'bg-slate-800 text-slate-500'}`}>
                {isEmergency ? 'EMERGENCY ROUTE' : 'OPTIMIZED PATH'}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center md:items-end">
            ...
            <span className={`font-mono text-lg ${isEmergency ? 'text-red-400 font-bold' : 'text-emerald-400'}`}>
              {routeData.best_route[routeData.best_route.length - 1].lat.toFixed(4)}°, {routeData.best_route[routeData.best_route.length - 1].lng.toFixed(4)}°
            </span>
            {isEmergency && emergencyHarbor && (
              <span className="text-xs text-red-300 font-bold mt-1">({emergencyHarbor.name})</span>
            )}
          </div>
        </div>
      )}
